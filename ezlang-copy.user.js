...
(function () {
  ...
  function getResultContainer() {
    const resultTitle = Array.from(document.querySelectorAll("div")).find(
      (element) => {
        return (element.textContent || "").trim() === RESULT_TITLE_TEXT;
      },
    );

    const card = resultTitle?.closest(
      ".rounded-xl.border.bg-card.text-card-foreground.shadow.my-6",
    );
    if (!card) {
      return null;
    }
  ...
  function setButtonState(
    button,
    { text, background, disabled = false, resetText },
  ) {
  ...
  function mountButton() {
    ...
    if (!document.getElementById(BUTTON_CONFIGS.source.id)) {
      convertButton.insertAdjacentElement(
        "afterend",
        createActionButton(BUTTON_CONFIGS.source, copySourceCode),
      );
    }
    ...
    if (!document.getElementById(BUTTON_CONFIGS.format.id)) {
      ...
      copyButton?.insertAdjacentElement(
        "afterend",
        createActionButton(BUTTON_CONFIGS.format, copyFormattedText),
      );
    }
  }
